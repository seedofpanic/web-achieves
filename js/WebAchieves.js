...
var WebAchieves = function (params) {
    var that = this;
    if (typeof $ == 'undefined') {
        var $ = jQuery;
    }
    ...
    var DOMAIN = params.domain || 'http://webachievs.ru/';
    ...
    this.achievesBlock = $(that.templates.box).appendTo('body');
    this.achievesBox = this.achievesBlock.find('.achieves');
    that.showdAll = false;
    this.achievesBlock.find('.controls .star').click(function () {
        if (that.showdAll) {
            that.achievesBox.html('');
            that.showdAll = false;
        } else {
            $.ajax({
                url: DOMAIN + 'api/visitor/' + params.domain_id + '/achieves/',
                type: 'post',
                ...
                success: function (data) {
                    that.achievesBox.html('');
                    $.each(data, function (key, val) {
                        val.achieved = val.achieved > 0 ? '' : 'not-achieved';
